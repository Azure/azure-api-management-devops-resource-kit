...
The utility supports the following arguments: 
 ...
 `--configFile`: Required. Points to a yaml file that controls the ARM templates generated by the Creator tool. The file contains a Creator Configuration object whose schema and related schemas are listed below.  
 `--[overrideApimFileName]`: Optional. If this value is provided then the creator will use this value to in place of the {apimServiceName} property name found in the config file. 
