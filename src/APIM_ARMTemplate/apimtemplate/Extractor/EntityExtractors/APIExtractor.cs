...
namespace Microsoft.Azure.Management.ApiManagement.ArmTemplates.Extract
{
    public class APIExtractor : EntityExtractor
    {
        ...
        public async Task<List<TemplateResource>> GenerateSingleAPIResourceAsync(string apiName, Extractor exc)
        {
            ...
            Console.WriteLine("------------------------------------------");
            Console.WriteLine("(Single) Extracting resources from {0} API:", apiName);
            ...
            foreach (string operationName in operationNames)
            {
                ...
                if(exc.operationBatchSize > 0 && opIndex > 0) {
                    if(opIndex >= 1 && opIndex <= numBatches - 1) {
                        ...
                        batchdependsOn = $"[resourceId('Microsoft.ApiManagement/service/apis/operations', parameters('{ParameterNames.ApimServiceName}'), '{oApiName}', '{batchOwners[opIndex - 1]}')]";
                        ...
                        Console.WriteLine("OwnerIndex is {0}, opIndex is {1}, batchSize is {2}, listSize is {3}", ownerIndex, opIndex, exc.operationBatchSize-1, operationNames.Length);
                        batchdependsOn = $"[resourceId('Microsoft.ApiManagement/service/apis/operations', parameters('{ParameterNames.ApimServiceName}'), '{oApiName}', '{batchOwners[ownerIndex]}')]";
