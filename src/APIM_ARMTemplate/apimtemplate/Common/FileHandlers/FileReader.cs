  ...
  public class FileReader
  {
    ...
    public async Task<string> RetrieveFileContentsAsync(string fileLocation)
    {
      ...
      if (isUrl)
      {
        ...
        {
          throw new Exception($"Unable to fetch remote file - {fileLocation}. Please ensure you IP is Whitelisted in the Swagger location (i.e. the Web App in Azure) and the authentication scheme is not on.");
        }
      }
      else
      {
