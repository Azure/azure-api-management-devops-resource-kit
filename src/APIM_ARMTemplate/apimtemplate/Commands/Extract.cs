...
namespace Microsoft.Azure.Management.ApiManagement.ArmTemplates.Extract
{
	public class ExtractCommand : CommandLineApplication
	{
		public ExtractCommand()
		{
			...
			var revisionName = this.Option("--revisionName <revisionName>", "Revision name", CommandOptionType.SingleValue);
			...
			this.OnExecute(async () =>
			{
				try
				{
					...
					string singleRevisionName = null;
					...
					if (singleApiName != null && revisionName.Values.Count > 0)
					{
						singleRevisionName = revisionName.Values[0].ToString();
					}
					...
					if (singleRevisionName != null)
					{
						Console.WriteLine($"Executing extraction for {singleApiName} API, Revision {singleRevisionName}");
					}
					else if (singleApiName != null && singleRevisionName == null)
					{
						Console.WriteLine($"Executing extraction for {singleApiName} API, All Revisions");
					}
					else
					{
					...
					Template apiTemplate = await apiExtractor.GenerateAPIsARMTemplateAsync(sourceApim, resourceGroup, singleApiName, singleRevisionName, policyXMLBaseUrl, fileFolder);
