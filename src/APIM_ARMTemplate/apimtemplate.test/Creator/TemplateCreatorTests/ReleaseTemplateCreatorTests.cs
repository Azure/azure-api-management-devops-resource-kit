...
namespace Microsoft.Azure.Management.ApiManagement.ArmTemplates.Test
{
    public class ReleaseTemplateCreatorTests
    {
        ...
        public void ShouldCreateReleaseTemplateResourceFromCreatorConfig()
        {
            ...
            // assert
            string releaseName = $"";
            Assert.Equal($"[concat(parameters('ApimServiceName'), '/{api.name};rev={api.apiRevision}/release-revision-{api.apiRevision}')]", releaseTemplateResource.name);
            ...
            Assert.Equal($"[resourceId('Microsoft.ApiManagement/service/apis', parameters('ApimServiceName'), '{api.name};rev={api.apiRevision}')]", releaseTemplateResource.properties.apiId);
        }
    }
}
